document.addEventListener("DOMContentLoaded", () => {
  async function login(username, password) {
    try {
      const response = await fetch("https://travelapps-asep-hendars-projects.vercel.app/login", {
        method: "POST",
        headers: {
...
document.addEventListener("DOMContentLoaded", () => {
  async function register(formData) {
    try {
      const response = await fetch('https://travelapps-asep-hendars-projects.vercel.app/registers', {
        method: 'POST',
        body: formData,
        credentials: 'include',
      });
