  ...
  schedules.forEach(schedule => {
      ...
      const carCard = `
          <div class="col-md-4 mb-4">
              <div class="card">
                  <img src="/assets/images/Isuzu_Elf.png" class="card-img-top" alt="${car.name}">
                  <div class="card-body">
...
export async function fetchSchedules(id_user) {
  try {
    const response = await fetch(`https://travelapps-asep-hendars-projects.vercel.app/schedules?id_user=${id_user}`, {
      credentials: 'include'
    });
  ...
  paginatedSchedules.forEach((schedule, index) => {
    ...
    const row = `
      <tr>
        ...
        <td><img src="/assets/images/rental-innova.png" alt="Car Image" style="max-width: 150px;"></td>
...
export async function searchSchedules(searchTerm) {
  ...
  try {
    const response = await fetch(`https://travelapps-asep-hendars-projects.vercel.app/schedules?${queryParams.toString()}`, {
      method: 'GET',
      credentials: 'include'
    });
...
export async function fetchSchedulesDetails(scheduleId) {
  try {
    const response = await fetch(`https://travelapps-asep-hendars-projects.vercel.app/schedules/${scheduleId}`, {
      method: 'GET',
      credentials: 'include'
    });
...
export async function editSchedules(schedulesId, formData) {
  try {
    const response = await fetch(`https://travelapps-asep-hendars-projects.vercel.app/schedules/${schedulesId}`, {
      method: 'PUT',
      body: formData,
      credentials: 'include'
    });
